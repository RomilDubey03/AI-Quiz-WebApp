...
const getCreatorDashboard = asyncHandler(async (req, res) => {
    ...
    const userQuizzes = await Quiz.find({ creator: userId })
        .select('title description status totalParticipants questions')
        .lean();
    ...
    const dashboardData = userQuizzes.map(quiz => ({
        quizId: quiz._id,
        title: quiz.title,
        description: quiz.description,
        status: quiz.status,
        totalQuestions: quiz.questions.length,
        uniqueParticipants: quiz.totalParticipants
    }));
...
const getQuizDetails = asyncHandler(async (req, res) => {
    ...
    const quiz = await Quiz.findOne({ 
        _id: quizId,
        creator: userId  // Ensure user owns this quiz
    }).select('title description questions status totalParticipants');
